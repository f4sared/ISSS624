...
# Step 6: Prepare the Spatial Weights
...
```{r}
wm_d72 <- dnearneigh(coords, 0, 72140, longlat = FALSE)
wm_d72
```
...
```{r}
plot(nga_wp_26391$geometry, border="black")
...
```{r}
# plays <- nb2listw(wm_d72, 
#                    style="B", 
#                    zero.policy = TRUE)
```

```{r}
# globalG.test(nga_wp$`wpt functional`, plays)
```
